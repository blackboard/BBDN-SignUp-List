...
var updateLogComments = { 'comments': 'test post log with full data' }
...
describe('[test_log_schema] Return what we just created', function () {
  it('should GET correctly', (done) => {
    chai
      .request(server)
      .get('/logs/' + logCreatedUUID).set('Cookie', 'sulToken=' + jwtTokenInstructor)
      .end((err, res) => {
        res.should.have.status(200)
        res.should.be.json
        done()
...
describe('[test_log_schema] Delete what we created', function () {
  it('should delete what we POSTed', (done) => {
    chai
      .request(server)
      .delete('/logs/' + logCreatedUUID).set('Cookie', 'sulToken=' + jwtTokenInstructor)
      .end((err, res) => {
